...
def jv_threshold(x):
    ...
    """
    rel_path = pathlib.Path('data', 'tools', 'math', 'special', 'jv_threshold.csv')
    abs_path = pkg.resource_filename('acoustic_camera', str(rel_path))
...
def spherical_jn_threshold(x):
    ...
    int
        Value of `n` in :math:`j_{n}(x)` past which :math:`j_{n}(x) \approx 0`.
    """
    rel_path = pathlib.Path('data', 'tools', 'math', 'special', 'spherical_jn_threshold.csv')
    abs_path = pkg.resource_filename('acoustic_camera', str(rel_path))
...
def ive_threshold(x):
    ...
    """
    rel_path = pathlib.Path('data', 'tools', 'math', 'special', 'ive_threshold.csv')
    abs_path = pkg.resource_filename('acoustic_camera', str(rel_path))
...
def spherical_jn_series_threshold(x, table_lookup=True, epsilon=1e-2):
    ...
    if table_lookup is True:
        rel_path = pathlib.Path('data', 'tools', 'math', 'special', 'spherical_jn_series_threshold.csv')
        abs_path = pkg.resource_filename('acoustic_camera', str(rel_path))
...
def jv_series_threshold(x):
    ...
    int
        Value of `n` in :math:`f_{n}(x)` past which :math:`f_{n}(x) \ge 1 - \epsilon`.
    """
    rel_path = pathlib.Path('data', 'tools', 'math', 'special', 'jv_series_threshold.csv')
    abs_path = pkg.resource_filename('acoustic_camera', str(rel_path))
